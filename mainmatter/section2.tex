...
\red{Start by explaining this formula:}
\begin{align}
    \delta T_B = \frac{T_S - T_\gamma}{1+z} \bigl(1-\exp^{-\tau}\bigr)  
\end{align}
\red{Then go into details with the following section, which still has to be fixed:}
...
\begin{align}
    ...
    \equiv \frac{g_1}{g_0} \exp \biggl(-\frac{T_\star}{T_S}\biggr),
\end{align}
...
Physical processes which lead to (de-)excitation of the neutral hydrogen spin state are:
\begin{enumerate}
...
The Einstein coefficients for each of these processes will be denoted $A$ for spontaneous emission, $B$ for emission/absorption from interaction with CMB, and two effective coefficients $C$ for collision interactions and $P$ for Lyman-$\alpha$ interactions.
...
The exact estimation of $T_S$ is achieved through numerics.
