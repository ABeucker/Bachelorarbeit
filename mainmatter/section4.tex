...
\begin{align}
    ...
    \geq \mathbb{I}(\theta;z),
\end{align}
and this explains why mutual information is the measure of informativeness mentioned in the beginning of this section. An optimal summary contains all needed information about the original data while being as low-dimensional as possible. Of course, a truly optimal summary is never found in practice. The part of the network in which the compression of data is the highest (in this case the output z) is often referred to as the bottleneck.
...
Let the input data have dimensions $x \in \R^m$. The CNN learns a function $s_\phi(x)$ which summarizes the data. It multiple types of layers to achieve this:
...
\red{Need to agree on how sampling is written for expectation}
...
\end{align}
One optimizes the flow by minimizing the forwards KL-divergence between the true and the approximated distribution, which as discussed ins section~\ref{subsubsec:meas_IT} is equivalent to minimizing the NLL:
\begin{align}
