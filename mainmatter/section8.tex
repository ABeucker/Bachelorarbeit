...
\end{figure}
The \red{per-epoch estimation of conditional entropy} comes with an associated error, here called the \textit{model error}. This is estimated from the distribution of the flow's log-probability of the testing set. \red{Begründen, maybe something from MCMC.} According to the CLT, one would expect the error to be estimated by the standard deviation. In this case, the log-probability values are distributed with heavy \red{left-sided} kurtosis, which inflates the value of the standard deviation. As an alternative estimate of error, a bootstrapping method is implemented. \red{Quote some source that cites this as best practice.} \red{The exact values used for bootstrapping for reproducibility.} \red{Something on interpretation of bootstrap results.}
