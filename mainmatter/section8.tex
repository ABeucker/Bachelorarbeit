...
\red{Comparability of loss curves? Since loss scaled with bottleneck dim}

One can observe a higher \red{seeding variance} for models with smaller bottleneck dimension in training and validation losses. \red{Quantify the seed variance for best validation loss here.} This indicates a higher training instability for models with smaller bottleneck dimension.
\begin{figure}
    \centering
...
Figures~\cref{fig:mi_bootstrap_bottle_4,fig:mi_bootstrap_bottle_6,fig:mi_bootstrap_bottle_15,fig:mi_bootstrap_bottle_16} show the estimated conditional entropy per epoch for each ensemble the model was trained on. Vertical lines again mark the epoch with best validation performance. The graphs' trends mirror those of the validation losses \red{Do they mirror valdidation losses? This would be expected.}, and all models reach a saturated estimate of the mutual information \red{in the epoch range...} The conditional entropy curves showcase a higher instability than the validation curves \red{Can I quantify this?}, which is here attributed to the smaller size of the test set (\red{put this interpretation somewhere else?}).
...
\end{figure}
The \red{per-epoch estimation of conditional entropy} comes with an associated error, here called the \textit{model error}. The model error stems from the variability in the log-probability of the test set samples as predicted by the flow. \red{Begründen, maybe something from MCMC.} 

According to the CLT, one would expect the error to be estimated by the standard deviation. In this case, the log-probability values are distributed with heavy \red{left-sided} kurtosis, which inflates the value of the standard deviation. As an alternative estimate of error, a bootstrapping method is implemented. \red{Quote some source that cites this as best practice.} \red{The exact values used for bootstrapping for reproducibility.} \red{Something on interpretation of bootstrap results.}

For each bottleneck dimension, the final conditional entropy estimate is made by selecting the model with best validation loss from each seed and averaging the conditional entropy for each of these models. The final error on the CE estimate is comprised of the model error and the \text{seed error}, stemming from the variability of results across different seeds. The final estimates with errors as shown in table~\ref{tab:CE_results}.
\begin{table}[htbp]
\centering
